...
The sweep is shelved rather than moved (operator decision, 2026-09-16); its scripts and workflow stay in this repository's history at `e49a81c`.
If it returns, its home should be a repository whose issues are the operator's own and can be private, so that private repositories can be named and the digest is unnecessary.
...
A fifth round corrected the inheritance rule itself: a consumer inherits whatever its pinned plugin revision enables, and `v0.7.0` is the first release whose `plugin.yaml` enables `pinact`, while every local consumer on 2026-09-16 pinned `v0.5.0` or `v0.6.0`.
The first inheritance rule had therefore pulled six repositories into scope that never opted in; the filter now reads `plugin.yaml` at the consumer's `ref` through the API (once per ref) and inherits only from a revision that enables `pinact`, which brought the sweep to 35 repositories (33 with a local entry plus two by inheritance from a revision that enables it) instead of the 39 the first rule produced; the count grows as consumers move to `v0.7.0` or later.
The same round noted trunk's mapping form of an enabled entry (`- pinact@x:` with `commands:`), which the regex now accepts.
A local drain before that push found that the notification's command line was not valid YAML once findings existed (a quoted scalar followed by text), which would have kept trunk from reading exactly the notification that mattered; the command is now one single-quoted scalar and the test parses a notification with findings through ruby's YAML parser.
The same drain found that a failed read of the pinned plugin revision passed as "does not enable pinact"; it is now a third verdict that lists the repository under "Not scanned".
