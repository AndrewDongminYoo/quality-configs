...
### Renaming Is Not the Only Way Out

This note first concluded that a separate name was the only route to the bundled `dart` definition.
That was wrong, and wrong in a specific way worth recording: the constraint it treated as fixed was one this repository had introduced hours earlier.

Raising the Python pin is what made `trunk-io/plugins` mandatory, and that source is what wins the name collision.
Bundling `runtimes/python` removes the dependency, and with the source gone nothing collides:

```log
autogenerated_definition_path: linters/dart
run: dart analyze --no-fatal-warnings --format=json ${target}   # the bundled definition
python@3.14.4: resolved from the bundled runtimes/ directory
```

`java@13.0.11` and every linter the profiles enable resolve from the CLI's built-in set, so `python` was the only gap.

Two costs come with dropping the source.
The bundled `linters/` copies are what a consumer then gets for `dart` and `toml-tidy`, which is the point, but every other linter definition comes from the CLI and therefore moves with `cli.version` rather than with a pinned `ref`.
And the bootstrap has to happen in one write: `trunk plugins add` needs a configuration it can already resolve, so a profile pinning `python@3.14.4` cannot be made resolvable by the command meant to add the plugin that defines it.

`scripts/test-plugin.sh` covers this as a fifth scenario, and it is the one assertion that reaches the bundled directories: removing the `runtimes/` copy fails the suite with exit 2, and restoring it passes.
